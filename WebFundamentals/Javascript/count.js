...
function count(lowNum, hiNum, step) {
	if ( lowNum === undefined ) {
		lowNum = 4;
	} else if ( hiNum === undefined ) {
		hiNum = 8;
	...
	if (lowNum > hiNum) {
		console.log("sorry, can you make sure first arguemnt is smaller than the second");
	} else {
		for ( var i = lowNum; i < hiNum; i = i + step ) {
			console.log(i);
		}
	}
}
